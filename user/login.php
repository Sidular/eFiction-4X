...
if (isset($_POST['submit']) && preg_match("!^[-a-z0-9_ ]{3,30}$!i", $_POST['penname'])) {
    ...
    $stmt = dbprepare("SELECT tableprefix, maintenance, sitekey, debug FROM " . $settingsprefix . "fanfiction_settings WHERE sitekey = ?");
    dbexecute($stmt, [$sitekey]);
    list($tableprefix, $maintenance, $sitekey, $debug) = dbrow($stmt);
    $tempdebug = $debug;
    $debug = 0;
    ...
    $stmt = dbprepare("SELECT *, " . _UIDFIELD . " as uid FROM " . _AUTHORTABLE . " LEFT JOIN " . $tableprefix . "fanfiction_authorprefs AS ap ON ap.uid = " . _UIDFIELD . " WHERE " . _PENNAMEFIELD . " = ?");
    dbexecute($stmt, [$_POST['penname']]);
    $passwd = dbassoc($stmt);
    ...
    if (verifyPassword($_POST['password'], $passwd['password'])) {
        // Upgrade weak MD5 hashes to bcrypt after a successful login.
        if (passwordNeedsRehash($passwd['password'])) {
            upgradePasswordHash(TABLEPREFIX, _UIDFIELD, $passwd['uid'], $_POST['password']);
        }

        if (isset($_POST['cookiecheck'])) {
            setcookie($sitekey . "_useruid", $passwd['uid'], time() + 60 * 60 * 24 * 30, "/");
            setcookie($sitekey . "_salt", hash("sha256", $passwd['email'] . $_POST['password']), time() + 60 * 60 * 24 * 30, "/");
        }
        if (!isset($_SESSION)) session_start();
        $_SESSION[$sitekey . "_useruid"] = $passwd['uid'];
        $_SESSION[$sitekey . "_salt"] = hash("sha256", $passwd['email'] . $_POST['password']);

        // Codeblocks are no longer executed via eval() for security. If a site
        // relies on custom login hooks, they must be implemented as static PHP
        // include files in a future release.
        //
